export default {
  ...
  setup: {
    ...
    webPort: '管理ポート',
    apiPort: 'API ポート',
    portHint: 'ポートは起動パラメータ（-web_port / -api_port）または config.ini で指定します。変更後は再起動が必要です',
  ...
  settings: {
    ...
    restartPortChanged: 'システムを再起動しました。ポートが変更されました：管理ポート {manage}、API ポート {api}。新しいアドレスでアクセスしてください。',
    portLockedByDocker: 'Docker 環境：ポートは起動コマンド（-web_port / -api_port）または環境変数で指定されるため、コンテナの起動パラメータで変更してください',
    restarting: '再起動中...',
